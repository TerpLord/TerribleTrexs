...
<body>
    ...
    <script>
        ...
        // Modify the drawTable function to include data-previousColor attribute
        function drawTable() {
            var tableCode = "";
            var colorTable = "";
                ...
                for (var i = 0; i < colorSize; i++) {
                    ...
                    for (var j = 0; j < 2; j++) {
                        if (j == 0) {
                            ...
                            // Add data-previousColor attribute to store previous value
                            let previousColor = colorMap.get(colorName);
                            colorTable += "<select id='colorDropdown_" + i +
                                "' onchange='checkSelection(this)' data-previousColor='" + previousColor + "'>";

                            // Adding options to the dropdown
                            for (let [name, color] of colorMap) {
                                colorTable += "<option value='" + color + "'" + (name === colorName ? " selected" :
                                    "") + ">" + name + "</option>";
                            }
                            colorTable += "</input>";
            ...
            $("#squareTable").on('click', 'td', function () {
                ...
                $("#colorSelectors").find("tr:eq(" + selectedRow + ") td:eq(1)").text(colorRows[selectedRow]
                    .join(", "));
